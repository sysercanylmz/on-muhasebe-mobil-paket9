    ...
    with get_connection() as conn:
        ...
        cur.execute(
            ...
            (islem_tipi, cari_id, kasa_id, kategori_id, tarih, belge_turu, belge_no,
             belge_tarihi, kdv_hesap_tipi, baslik, aciklama, tutar, kdv_orani, kdv_tutari,
...
def _islem_params(data):
    return (
        ...
        data.get("kdv_hesap_tipi") or "dahil",
        data.get("baslik"),
        data.get("aciklama"),
        data.get("tutar"),
        data.get("kdv_orani"),
    ...
    return fetch_all(
        ...
        SELECT
            CASE
                ...
                ELSE islem_tipi
            END AS islem_tipi,
            COALESCE(SUM(tutar), 0) AS matrah,
            COALESCE(SUM(kdv_tutari), 0) AS kdv_tutari,
            COALESCE(SUM(toplam_tutar), 0) AS toplam_tutar
        FROM islem_kayitlari
        WHERE kdv_tutari > 0 OR kdv_orani > 0
        GROUP BY
            CASE
                WHEN islem_tipi IN ('gelir', 'tahsilat') THEN 'gelir/tahsilat'
                WHEN islem_tipi IN ('gider', 'odeme') THEN 'gider/odeme'
...
# -----------------------------------------------------------------------------
# Vergi tahmini raporları
# -----------------------------------------------------------------------------
def _year_param(year):
    return str(int(year))


def vergi_aylik_kdv_ozeti(year):
    return fetch_all(
        """
        SELECT
            CAST(strftime('%m', tarih) AS INTEGER) AS ay,
            COALESCE(SUM(CASE WHEN islem_tipi = 'gelir' THEN kdv_tutari ELSE 0 END), 0) AS hesaplanan_kdv,
            COALESCE(SUM(CASE WHEN islem_tipi = 'gider' THEN kdv_tutari ELSE 0 END), 0) AS indirilecek_kdv,
            COALESCE(SUM(CASE WHEN islem_tipi = 'gelir' THEN tutar ELSE 0 END), 0) AS gelir_matrah,
            COALESCE(SUM(CASE WHEN islem_tipi = 'gider' THEN tutar ELSE 0 END), 0) AS gider_matrah
        FROM islem_kayitlari
        WHERE substr(tarih, 1, 4) = ?
          AND islem_tipi IN ('gelir', 'gider')
        GROUP BY CAST(strftime('%m', tarih) AS INTEGER)
        ORDER BY ay
        """,
        (_year_param(year),),
    )
...
def vergi_yillik_kar_ozeti(year):
    row = fetch_one(
        """
        SELECT
            COALESCE(SUM(CASE WHEN islem_tipi = 'gelir' THEN tutar ELSE 0 END), 0) AS gelir_matrah,
    ...
    gelir = float(row["gelir_matrah"] or 0)
    gider = float(row["gider_matrah"] or 0)
    return {
        "gelir_matrah": gelir,
        "gider_matrah": gider,
        "kar": gelir - gider,
        "hesaplanan_kdv": float(row["hesaplanan_kdv"] or 0),
        "indirilecek_kdv": float(row["indirilecek_kdv"] or 0),
    }


def vergi_gecici_donem_ozeti(year):
    rows = fetch_all(
        """
        SELECT
            CAST(strftime('%m', tarih) AS INTEGER) AS ay,
    ...
    monthly = {int(row["ay"]): row for row in rows if row["ay"] is not None}

    result = []
    cumulative_profit_before = 0.0
    previous_tax_estimate = 0.0
    try:
        from app.tax import QUARTERS, income_tax_2026_non_wage
    except Exception:
        QUARTERS = []
        income_tax_2026_non_wage = None

    for no, name, months, month_label in QUARTERS:
        gelir = sum(float(monthly.get(m, {}).get("gelir_matrah", 0) or 0) for m in months)
        gider = sum(float(monthly.get(m, {}).get("gider_matrah", 0) or 0) for m in months)
        kar = gelir - gider
        cumulative_profit = cumulative_profit_before + kar

        if income_tax_2026_non_wage:
            cumulative_tax = income_tax_2026_non_wage(cumulative_profit).tax
        else:
            cumulative_tax = 0.0
        donem_tahmini = max(cumulative_tax - previous_tax_estimate, 0.0)
        ...
        result.append({
            ...
            "gider_matrah": gider,
            "kar": kar,
            "kumulatif_kar": cumulative_profit,
            "kumulatif_vergi": cumulative_tax,
            "donem_tahmini_vergi": donem_tahmini,
        })

        cumulative_profit_before = cumulative_profit
        previous_tax_estimate = cumulative_tax
