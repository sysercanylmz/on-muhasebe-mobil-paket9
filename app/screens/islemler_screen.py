...
from app.helpers import is_official_document, kdv_hesapla, money, number_for_input, to_float, today_str, ui_label, ui_options, ui_value
...
KDV_HESAP_TIPLERI = ui_options("kdv_hesap_tipi")
...
TIP_DEFAULTS = {
    "gelir": {"belge_turu": "belgesiz", "kdv_orani": "20", "kdv_hesap_tipi": "dahil", "odeme_durumu": "odendi"},
    "gider": {"belge_turu": "belgesiz", "kdv_orani": "20", "kdv_hesap_tipi": "dahil", "odeme_durumu": "odendi"},
    "tahsilat": {"belge_turu": "dekont", "kdv_orani": "0", "kdv_hesap_tipi": "dahil", "odeme_durumu": "odendi"},
    "odeme": {"belge_turu": "dekont", "kdv_orani": "0", "kdv_hesap_tipi": "dahil", "odeme_durumu": "odendi"},
}
...
class IslemlerScreen(RefreshableScreen):
    ...
    def _build_form(self):
        ...
        self.toplam_tutar = form_input("Tutar")
        self.kdv_hesap_tipi = form_spinner(KDV_HESAP_TIPLERI, ui_label("kdv_hesap_tipi", "dahil"))
        self.kdv_orani = form_input("KDV oranı (%)", text="20")
        self.kdv_preview = label("", size=12, color=MUTED, height=46)
        ...
        fields = [
            ...
            ("Tutar Giriş Şekli", self.kdv_hesap_tipi),
            ("KDV Oranı (%)", self.kdv_orani),
            ("KDV Özeti", self.kdv_preview),
        ...
        self.toplam_tutar.bind(text=lambda *_: self.update_kdv_preview())
        self.kdv_orani.bind(text=lambda *_: self.update_kdv_preview())
        self.kdv_hesap_tipi.bind(text=lambda *_: self.update_kdv_preview())
    ...
    def apply_type_view(self, reset_fields: bool = False):
        ...
        self.toplam_tutar.hint_text = "Tutar"
        ...
        if reset_fields:
            ...
            self.kdv_hesap_tipi.text = ui_label("kdv_hesap_tipi", defaults.get("kdv_hesap_tipi", "dahil"))
    ...
    def update_kdv_preview(self):
        if not hasattr(self, "kdv_preview"):
            return
        if self.active_tip in ("tahsilat", "odeme"):
            self.kdv_hesap_tipi.text = ui_label("kdv_hesap_tipi", "dahil")
            self.kdv_orani.text = "0"
            self.kdv_preview.text = "Tahsilat ve ödeme kayıtlarında KDV hesaplanmaz."
            return
        try:
            tutar_girdisi = to_float(self.toplam_tutar.text)
            kdv_orani = to_float(self.kdv_orani.text)
            hesap_tipi = ui_value("kdv_hesap_tipi", self.kdv_hesap_tipi.text) or "dahil"
            matrah, kdv_tutari, genel_toplam = kdv_hesapla(tutar_girdisi, kdv_orani, hesap_tipi)
            self.kdv_preview.text = f"Matrah: {money(matrah)}\nKDV: {money(kdv_tutari)} • Genel Toplam: {money(genel_toplam)}"
        except Exception:
            self.kdv_preview.text = "KDV hesaplaması için geçerli tutar ve oran gir."
    ...
    def collect(self):
        ...
        if self.active_tip in ("tahsilat", "odeme"):
            kdv_orani = 0
            kdv_hesap_tipi = "dahil"
            tutar = tutar_girdisi
            kdv_tutari = 0
            toplam = tutar_girdisi
        else:
            kdv_orani = to_float(self.kdv_orani.text)
            if kdv_orani < 0:
                raise ValueError("KDV oranı negatif olamaz.")
            kdv_hesap_tipi = ui_value("kdv_hesap_tipi", self.kdv_hesap_tipi.text) or "dahil"
            if kdv_hesap_tipi not in ("dahil", "haric"):
                raise ValueError("Tutar giriş şeklini KDV Dahil veya KDV Hariç seçmelisin.")
            tutar, kdv_tutari, toplam = kdv_hesapla(tutar_girdisi, kdv_orani, kdv_hesap_tipi)
    ...
    def edit(self, row):
        ...
        hesap_tipi = row["kdv_hesap_tipi"] or "dahil" if "kdv_hesap_tipi" in row.keys() else "dahil"
        self.kdv_hesap_tipi.text = ui_label("kdv_hesap_tipi", hesap_tipi)
        if hesap_tipi == "haric":
            self.toplam_tutar.text = number_for_input(row["tutar"])
        else:
    ...
    def detail(self, row):
        ...
        message = (
            ...
            f"Matrah: {money(row['tutar'])}\n"
            f"KDV: %{number_for_input(row['kdv_orani'])} / {money(row['kdv_tutari'])}\n"
            f"Genel Toplam: {money(row['toplam_tutar'])}\n"
            f"KDV Giriş: {ui_label('kdv_hesap_tipi', row['kdv_hesap_tipi'] or 'dahil')}\n"
    ...
    def reload_list(self):
        ...
        for row in rows:
            ...
            meta = f"{row['tarih']} • {ui_label('belge_turu', row['belge_turu'])} • KDV %{number_for_input(row['kdv_orani'])} • {ui_label('odeme_durumu', row['odeme_durumu'])}"
